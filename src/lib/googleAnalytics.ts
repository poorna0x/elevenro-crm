...
function isInternalNonMarketingPath(pathname: string): boolean {
  return pathname.startsWith('/product-verify') || pathname.startsWith('/technician-id/');
}
...
export function shouldSendToGoogleAnalytics(pathname: string): boolean {
  if (!getGoogleAnalyticsMeasurementId()) return false;
  if (!isPublicMarketingPathForGa(pathname)) return false;
  if (isStaffAuthSessionForGa()) return false;
  return true;
}
...
export function trackGoogleAnalyticsPageView(pagePath: string): void {
  if (!shouldSendToGoogleAnalytics(pagePath.split('?')[0] || '/')) return;

...
export async function fetchBookingHubSettings(opts?: {
  ...
  const { data, error } = await dbFrom(BOOKING_HUB_SETTINGS_TABLE)
...
export async function fetchBookingServiceHubs(opts?: {
  ...
  const runSelect = (columns: string) => {
    let query = dbFrom(BOOKING_SERVICE_HUBS_TABLE)
      .select(columns)
...
export async function createBookingServiceHub(input: {
  ...
  let { data, error } = await dbFrom(BOOKING_SERVICE_HUBS_TABLE)
    .insert(payload)
    .select(PUBLIC_COLUMNS)
    .single();
  if (error && String(error.message || '').toLowerCase().includes('polygon')) {
    delete payload.polygon;
    const retry = await dbFrom(BOOKING_SERVICE_HUBS_TABLE).insert(payload).select(PUBLIC_COLUMNS_NO_POLYGON).single();
    data = retry.data;
    error = retry.error;
  }
  if (error) {
...
export async function updateBookingServiceHub(
  ...
  const { data, error } = await dbFrom(BOOKING_SERVICE_HUBS_TABLE)
    .update(payload)
    .eq('id', id)
    .select(PUBLIC_COLUMNS)
    .single();
...
export async function deleteBookingServiceHub(id: string): Promise<{ error: string | null }> {
  const { error } = await dbFrom(BOOKING_SERVICE_HUBS_TABLE).delete().eq('id', id);
